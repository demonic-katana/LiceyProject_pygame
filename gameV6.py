...
for n in range(1, 5):
    ...
    exec(f"level_{n} = list(map(str.strip, open(f'data/levels/level_0{n}.map', mode='r', encoding='utf8').readlines()))")
                    ...
                    if event.type == pygame.QUIT:
                        ...
                        if event.button == 1:
                            ...
                            level = [n for i, j, n in
                                     ...
                                     e in range(i, j)][0]
                            if 1 < event_pos[0] < 142 and (str(level) in progress or level == 1):
